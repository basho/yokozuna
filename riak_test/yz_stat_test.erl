...
populate_data_and_wait(Pid, Cluster, Bucket, Index, Count) ->
    ...
    yz_rt:wait_until(Cluster, fun check_queue_capacity_stats/1),
    yz_rt:drain_solrqs(Cluster),
    Search = <<"text:*">>,
    F = fun(_) ->
...
check_queue_capacity_stats(Node) ->
    ...
    QDCapacity = proplists:get_value(?STAT_NAME([queue, capacity]), Stats),
    QDCapacityValue = proplists:get_value(value, QDCapacity),
