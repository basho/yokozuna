...
confirm() ->
    ...
    yz_rt:write_data(Cluster, OldPid, ?INDEX1,
    ...
    %% test query count
    yz_rt:verify_num_found_query(Cluster, ?INDEX1, KeyCount),
    ...
    yz_rt:write_data(Cluster, Pid, ?INDEX2, {?SCHEMANAME, ?TEST_SCHEMA},
                           ?BUCKET2, GenKeys),
...
test_extractor_with_aae_expire(Cluster, Index, Bucket, Packet) ->
    ...
    {Host, Port} = rt:select_random(yz_rt:host_entries(
                                               rt:connection_info(
                                                 Cluster))),
    ...
    yz_rt:override_schema(APid, Cluster, Index, ?SCHEMANAME,
                                ?TEST_SCHEMA_UPGRADE),

    {ok, "200", RHeaders, _} = yz_rt:http(get, URL, [{"Content-Type", CT}],
                                                [], []),
...
test_bad_extraction(Cluster) ->
    ...
    %%
    yz_rt:create_indexed_bucket_type(Cluster, ?TYPE3, ?INDEX3, ?SCHEMANAME),
    %%
    ...
    {Host, Port} = rt:select_random(
        yz_rt:host_entries(rt:connection_info(Cluster))),
    ...
    {ok, "204", _, _} = yz_rt:http(put, URL, Headers, Data),
    %%
    %% The put should pass, but because it's "bad data", there should
    %% be no data in Riak.
    %%
    yz_rt:verify_num_found_query(Cluster, ?INDEX3, 0),
    %%
