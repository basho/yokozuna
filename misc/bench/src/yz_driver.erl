...
run({search, Qry, Params}, KG, VG, S) ->
    run({search, Qry, Params, ?DONT_VERIFY}, KG, VG, S);

run({search, Qry, Params, Expected}, _, _, S=#state{surls=URLs}) ->
    Base = get_base(URLs),
    ...
    %%               _ -> [{fl, FL}|Params1]
    %%           end,
    Params2 = mochiweb_util:urlencode([{q, Qry}, {wt, <<"json">>}|Params]),
